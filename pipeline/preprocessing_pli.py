...
TIF_FOLDER = "Sample1_PLI/In_plane"  # input folder containing *.tif files
...
def process_phi(phi, downsample=1):
    ...
    keep_labels = {
        r.label for r in regions if r.area >= area_thresh or r.area == largest_area
    }
...
def save_qc_figure(
    ...
    axes[1, 2].imshow(
        phi_masked, cmap="hsv", origin="lower", vmin=-np.pi / 2, vmax=np.pi / 2
    )
...
if MODE == "tif_folder":
    ...
    print(
        f"[{MODE}] phi range: {phi.min():.3f} to {phi.max():.3f}  "
    ...
    print(
        f"Tissue mask covers {coherent_mask.sum()} pixels "
