...
print(f"Smooth : {SMOOTH_SIGMA} voxels = {SMOOTH_SIGMA * zooms[0] * 1000:.0f} µm")
...
for z in range(shape[2]):
    ...
    print(
        f"  Valid after coherence filter: "
        f"{valid_tissue.sum():,} "
    ...
    chosen = rng.choice(len(seed_coords), size=n_seeds, replace=False)
    ...
    def lookup_2d(pts_mm):
        xi = np.clip(np.round(pts_mm[:, 0] / zooms[0]).astype(np.int32), 0, x_max)
        yi = np.clip(np.round(pts_mm[:, 1] / zooms[1]).astype(np.int32), 0, y_max)
        dx = dirx[xi, yi]
        dy = diry[xi, yi]
    ...
    def track_one_dir_2d(batch, sign=1.0):
        ...
        return [
            np.array(s, dtype=np.float32) if len(s) >= MIN_STEPS // 2 else None
            for s in streams
        ]
    ...
    for b in range(n_batches):
        ...
        pct = 100 * b1 / n_seeds
        print(
            f"  batch {b + 1}/{n_batches} ({pct:.0f}%)  sl: {len(slice_streamlines):,}",
            flush=True,
        )
        del fwd, bwd
        gc.collect()
...
if all_streamlines:
    ...
    print(
        f"Length : min={lengths.min()}  max={lengths.max()}  "
